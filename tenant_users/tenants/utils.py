...
def create_public_tenant(domain_url, owner_email, **owner_extra):
    ...
    TenantModel = get_tenant_model()
    TenantDataModel = get_tenant_domain_model()
    public_schema_name = get_public_schema_name()

    if TenantDataModel.objects.filter(schema_name=public_schema_name).first():
    ...
    if TENANT_SCHEMAS:
        ...
        public_tenant = TenantModel.objects.create(name='Public Tenant',
                                                   owner=profile)
